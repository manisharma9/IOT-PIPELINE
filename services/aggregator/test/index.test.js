"use strict";

const assert = require("node:assert/strict");
const test = require("node:test");
const { createApp } = require("../src/index");

function proposalRow(overrides = {}) {
  return {
    id: 1,
    event_time: "2026-05-24T18:00:00.000Z",
    created_at: "2026-05-24T17:56:00.000Z",
    signal_id: "signal-001",
    dso_id: "dso-dublin",
    community_id: "community-dublin-north",
    household_id: null,
    device_id: null,
    proposal_type: "load_reduction",
    requested_action: "reduce_load",
    proposed_action: "reduce_ev_charging",
    target_kw: 2.5,
    start_time: "2026-05-24T18:00:00.000Z",
    end_time: "2026-05-24T19:00:00.000Z",
    priority: "medium",
    status: "proposed",
    reason: "Reduce flexible EV charging demand during the grid signal window.",
    correlation_id: "signal-001",
    decision_payload: {
      no_execution: true
    },
    audit_payload: {
      no_execution: true
    },
    source_grid_signal: {
      raw: true
    },
    source_ieee20305_payload: {
      raw: true
    },
    ...overrides
  };
}

function listen(app) {
  return new Promise((resolve) => {
    const server = app.listen(0, () => {
      const address = server.address();
      resolve({
        server,
        url: `http://127.0.0.1:${address.port}`
      });
    });
  });
}

function close(server) {
  return new Promise((resolve, reject) => {
    server.close((error) => {
      if (error) {
        reject(error);
        return;
      }
      resolve();
    });
  });
}

test("GET /health reports proposal-only aggregator status", async () => {
  const app = createApp({
    pool: {
      query: async () => ({
        rows: []
      })
    }
  });
  const { server, url } = await listen(app);

  try {
    const response = await fetch(`${url}/health`);
    const body = await response.json();

    assert.equal(response.status, 200);
    assert.equal(body.status, "ok");
    assert.equal(body.service, "aggregator");
    assert.equal(body.command_execution_enabled, false);
    assert.equal(body.publishes.proposals, "dispatch.command.proposed");
  } finally {
    await close(server);
  }
});

test("GET /dispatch/proposals returns safe response", async () => {
  const app = createApp({
    pool: {
      query: async () => ({
        rows: [proposalRow()]
      })
    }
  });
  const { server, url } = await listen(app);

  try {
    const response = await fetch(`${url}/dispatch/proposals`);
    const body = await response.json();

    assert.equal(response.status, 200);
    assert.equal(body.status, "ok");
    assert.equal(body.count, 1);
    assert.equal(body.proposals[0].status, "proposed");
    assert.equal(body.proposals[0].proposed_action, "reduce_ev_charging");
    assert.equal(body.proposals[0].source_grid_signal, undefined);
    assert.equal(body.proposals[0].source_ieee20305_payload, undefined);
  } finally {
    await close(server);
  }
});

test("GET /dispatch/proposals/:id returns one proposal", async () => {
  const app = createApp({
    pool: {
      query: async () => ({
        rows: [proposalRow({ id: 7 })]
      })
    }
  });
  const { server, url } = await listen(app);

  try {
    const response = await fetch(`${url}/dispatch/proposals/7`);
    const body = await response.json();

    assert.equal(response.status, 200);
    assert.equal(body.status, "ok");
    assert.equal(body.proposal.id, 7);
    assert.equal(body.proposal.status, "proposed");
  } finally {
    await close(server);
  }
});
