...
| --- | --- | --- | --- | --- | --- |
| Communities and households | Example household telemetry and simulated devices | `examples/`, `services/shelly-simulator`, `services/enode-simulator` | Real enrolled customers/devices | Simulated | Consent model, enrollment, identity |
| HTTP telemetry | Gateway to ingestion API | `services/security-gateway`, `services/ingestion-api` | API Gateway to private service | Local-ready | API domain, auth model |
...
| SLM semantic connector | Deterministic plus optional Ollama SLM | `services/semantic-connector` | ECS/Fargate plus model endpoint | Complete locally | Production model hosting policy |
| SAREF4ENER | Deterministic semantic mapping | `services/semantic-connector/src/saref4ener-mapping.js` | Same service/library | Complete locally | Vocabulary governance |
...
| ENERSHARE/dataspace export | Minimized local export foundation | `services/dataspace-export`, `connectors/` | Real connector or EDC integration | Local foundation | Credentials, contract negotiation |
